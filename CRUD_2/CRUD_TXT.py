...
def carregar_biblioteca():
    biblioteca = {}
    try:
        with open(r'CRUD_2\biblioteca.txt', 'r') as file:
            for line in file:
                isbn, titulo, autor, genero = line.strip().split(' | ')
                biblioteca[isbn] = {
                    "Título": titulo,
                    "Autor": autor,
                    "Gênero": genero
...
# Função para salvar a biblioteca no arquivo
def salvar_biblioteca(biblioteca):
    with open(r'CRUD_2\biblioteca.txt', 'w') as file:
        for isbn, info in biblioteca.items():
            file.write(f"{isbn} | {info['Título']} | {info['Autor']} | {info['Gênero']}\n")
